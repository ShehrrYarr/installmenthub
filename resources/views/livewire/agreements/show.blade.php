                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                @foreach ($agreement->schedules as $schedule)
                                    <tr>
                                        ...
                                        <td class="px-4 py-2.5 text-right text-gray-900 dark:text-white">{{ number_format((float) $schedule->total_due, 0) }}</td>
                                        <td class="px-4 py-2.5 text-right text-gray-500">{{ number_format((float) $schedule->amount_paid, 0) }}</td>
                    ...
                    <div class="space-y-2">
                        @forelse ($agreement->payments as $payment)
                            <div class="flex items-center justify-between text-sm rounded-lg bg-gray-50 dark:bg-gray-900/40 px-3 py-2">
                                ...
                                <span class="font-medium text-emerald-600 dark:text-emerald-400">Rs. {{ number_format((float) $payment->amount, 0) }}</span>
                            </div>
                        @empty
            ...
            <div class="space-y-6">
                <div class="rounded-2xl border border-walnut-200/60 dark:border-walnut-900/60 bg-walnut-50/70 dark:bg-walnut-900/40 backdrop-blur-xl shadow-lg p-5 space-y-3">
                    ...
                    <dl class="space-y-2 text-sm">
                        <div class="flex justify-between"><dt class="text-walnut-600/70 dark:text-walnut-200/70">Product Price</dt><dd class="text-walnut-900 dark:text-walnut-200">Rs. {{ number_format((float) $agreement->product_price, 0) }}</dd></div>
